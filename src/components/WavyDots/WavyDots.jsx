...
const WavyDots = ({ color1, color2 }) => {
    ...
    const [cameraPosition, setCameraPosition] = useState(new THREE.Vector3(...getCameraPosition(location.pathname)));

    useEffect(() => {
        setCameraPosition(new THREE.Vector3(...getCameraPosition(location.pathname)));
    }, [location.pathname]);

    return (
        <div className='anim'>
            <Suspense fallback={<div>Loading...</div>}>
                <AnimationCanvas color1={color1} color2={color2} targetPosition={cameraPosition} />
            </Suspense>
        </div>
    );
};
